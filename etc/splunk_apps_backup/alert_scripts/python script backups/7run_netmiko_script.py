...
def execute_commands(net_connect, commands, ip_address, state=''):
    ...
    for command in commands:
        if command.strip():  # Check if the command is not just whitespace
            state_info = f" ({state})" if state else ""
            title = f"\n{'=' * 20}\nExecuting \"{command.strip()}\" on {ip_address}{state_info}\n{'=' * 20}\n"
...
def execute_config_commands(net_connect, commands, ip_address):
    ...
    for command in commands:
        if command.strip():  # Check if the command is not just whitespace
            title = f"\nSending config command \"{command.strip()}\" to {ip_address}\n"
            output = net_connect.send_config_set(command.strip())
            results += title + output
